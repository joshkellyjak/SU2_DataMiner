...
class FlameletSolver_Cantera:
    ...
    def saveSolution(self):
        ...
        if self.__solution_fileformat == "csv":
            self._thermochemical_solution.to_csv(filename_plus_folder+".csv",index=False)
        else:
            self._flameletSolution.save("%s.%s" % (filename_plus_folder, "yaml"), overwrite=True)
        
        return
    
    def setSolutionFormat(self, format_filename:str="csv"):
        if format_filename not in self.__supported_formats:
            raise Exception("Flamelet files can only be saved as " + ",".join((s for s in self.__supported_formats)))
        self.__solution_fileformat= format_filename
        return 
    ...
    def loadSolution(self, flameletFileName:str):
        ...
        file_extension = flameletFileName.split(".")[-1]
        if file_extension=="csv":
            self._from_csv(flameletFileName)
        else:
            self._from_yaml(flameletFileName)

        self._canteraSolution.Y = self._flameletSolution.Y[:,0]
        ...
        mixtureFraction = self._canteraSolution.mixture_fraction(self._Config.GetFuelString(), self._Config.GetOxidizerString())
        if self._Config.DefineMixtureStatus():
            self.setMixtureStatus(mixtureFraction)
        else:
            self.setMixtureStatus(equivalenceRatio)
        
        self._loadSolverSpecificData()
        self._prepareFlameletSimulation()
        self._extractSolutionDataForOutput()
        return 
    ...
    def _from_csv(self, csv_file_name:str):
        self._thermochemical_solution = pd.read_csv(csv_file_name)

        flameSolutionDataFrame = pd.DataFrame()
        flameSolutionDataFrame["grid"] = self._thermochemical_solution[FGMVars.Distance.name]
        flameSolutionDataFrame["velocity"] = self._thermochemical_solution[FGMVars.Velocity.name]
        flameSolutionDataFrame["T"] = self._thermochemical_solution[FGMVars.Temperature.name]
        flameSolutionDataFrame["D"] = self._thermochemical_solution[FGMVars.Density.name]
        for sp in self._canteraSolution.species_names:
            flameSolutionDataFrame["Y_%s" % sp] = self._thermochemical_solution["Y-%s" % sp]
        self._initial_grid = flameSolutionDataFrame["grid"]

        #self._initializeFlameletSolver()
        self._flameletSolution.set_initial_guess(data=flameSolutionDataFrame)
        return 
    
    def _from_yaml(self, yaml_file_name:str):
        #self._initializeFlameletSolver()
        self._flameletSolution.restore(yaml_file_name)
        #self._extractSolutionDataForOutput()
        return 
...
class CounterFlowDiffusionFlameSolver(FlameletSolver_Cantera):
    ...
    def _prepareReactants(self):
        ...
        self.__fuel_velocity = self.__oxidizer_density * self.__strain_rate / (3*self.__fuel_density+self.__oxidizer_density)
        self.__oxidizer_velocity = self.__strain_rate - self.__fuel_velocity
        return
